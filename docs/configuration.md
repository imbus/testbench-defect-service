...
|--------|------|-------------|---------|
| `client_class` | String | Identifies the client to load. See supported formats below. | `"JsonlDefectClient"` |
| `client_config_path` | String | Path to a separate `.toml` config file containing the client config. When omitted, the `[testbench-defect-service.client_config]` section of the main config file is used. | — |
...
| `port` | Integer | Port number to listen on. | `8030` |
...
The built-in clients support running shell commands before and after TestBench syncs defects, configured under a `commands` subsection.
